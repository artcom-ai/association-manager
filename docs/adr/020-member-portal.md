...
## Addendum: self-registration + onboarding profile editing (Release 0.3, ELESYTH real-data follow-up)

A follow-up request explicitly reopened this ADR's own "self-service profile editing and member self-registration are explicitly deferred" decision above, asking for a simple registration form (email/first name/last name/password) that leads directly into the Portal, where the member fills in their custom fields and submits for admin/secretariat approval. Prompted by ELESYTH's own real data now living in the system (531 imported MemberPress members) and the roadmap's Epic 5 (Member Experience) being explicitly called essential - not a hypothetical future need anymore.

**Classification: Module, not Implementation**, despite the request being framed as "for ELESYTH particular" - the registration mechanism, profile editing, and approval workflow are all generically reusable (any association wants "someone registers, fills their profile, an admin reviews"); only the specific fields being filled in are ELESYTH's own, and those were already handled by the pre-existing `FieldRegistry` extension point (ADR-021) with zero new implementation-side code needed.

**Editing is scoped to onboarding only, not reopened generally.** `PortalService::canEditProfile(Member $member)` returns true only for `candidate`/`pending_approval` - once a member reaches `active`, the Portal reverts to this ADR's original read-only view ("contact the association to update"). This deliberately narrows the reopened decision to exactly what was asked (fill in a profile before approval) rather than undoing the broader "no full profile editing" position for already-approved members.

**A new `MemberStatus::PENDING_APPROVAL`** sits between `candidate` (registered, profile not yet submitted) and `active` (admin approved) - a plain new status value (no migration; the column is already a free `VARCHAR`), with `candidate ⇄ pending_approval` and `pending_approval → active/inactive` transitions registered in `MemberStatusRegistry`. This gives the admin a real signal ("who actually finished onboarding") distinct from "who signed up and never came back," which a single `candidate` bucket couldn't express.

**Registration reuses `wp_insert_user()` directly** (role `subscriber`, no new custom role) via a new `MemberService::registerNewMember()` - validates email/password/names, throws `MemberRegistrationException` (same field-key → messages shape as `Core\Fields\FieldValidationException`) on failure, otherwise calls `createMember()` under the hood so a self-registered member fires the exact same `association_manager_member_status_changed` hook (and thus the existing `admin_new_member` notification) as every other member-creation path in this codebase.

**The approval-request notification reuses the existing engine entirely** - no new dispatch code. `PortalService::submitForApproval()` just saves field values then calls the already-public `MemberService::transitionStatus()`, which already fires the generic status-changed hook; `NotificationsModule::handleMemberStatusChanged()` gained one new branch (`pending_approval` → notify `admin_email`, mirroring `admin_new_member`'s recipient, distinct from the member-facing branches below it) instead of any new dispatch plumbing. `submitForApproval()` is deliberately idempotent on status - resubmitting while already `pending_approval` just updates field values without attempting a same-status transition (which `transitionStatus()` rejects outright).

**The Profile section's edit form reuses `Core\Fields\Admin\FieldRenderer`** (previously admin-form-only, called directly from `EditMemberPage`) instead of building a second field-rendering implementation - the same input-type-per-`FieldDefinition::type` logic applies identically to a member editing their own profile.

**Two different error-round-trip mechanisms, deliberately**: the registration form uses a random-token-keyed transient (`am_register_error_{token}` in the URL) since the visitor has no session yet to key by; the profile-submission form uses a `get_current_user_id()`-keyed transient (`am_profile_errors_{user_id}`), matching `ImportPage`'s existing per-admin-user transient pattern, since the member is already authenticated at that point. Both avoid inventing a new session/flash-message mechanism for what's fundamentally the same "redisplay the form with errors after a redirect" problem `ImportPage` already solved once.

**New migration 017** seeds the `member_submitted_for_approval` template (same insert-if-not-exists pattern as migrations 009/014), not the ELESYTH plugin - the event key and its existence are a Core/Module concern; only the Greek *content* of admin-facing templates would be implementation-specific, and this particular template is genuinely generic English admin copy (no Greek override was requested for it).

### Consequences

Positive:

- Registration → profile-fill → approval is now a real, working path for a brand-new member, not just for admin-imported ones - closing the most-cited gap from the "what does the Members module actually offer" discussion.
- Zero new Core notification infrastructure - the entire admin-facing "someone needs review" alert is two `match` arms and a seeded template, riding entirely on hooks that already existed.
- 21 new tests (registration validation/success paths, status-transition additions, `canEditProfile`/`submitForApproval`, the new notification routing) - 216 total, all passing, no regression in the 3+ prior addenda's coverage.

Negative:

- Registration's redirect-to-account-page depends on whoever places `[association_manager_register redirect="..."]` setting that attribute correctly - there's no automatic discovery of "the page with the Portal shortcode on it," matching the same "admin must already know the ID/URL" rough edge this ADR already accepted for "link WP account" and Certificates' "Issue Certificate" page.
- No email verification step exists - a registrant is logged in immediately after submitting valid email/password/names, with no confirmation-link flow. Acceptable given the admin approval step downstream still gates `active` status, but worth flagging if email ownership verification ever becomes a real requirement.
- `wp_insert_user()`/`wp_set_current_user()`/`wp_set_auth_cookie()` are WP-glue with no real-WordPress test coverage (new `tests/bootstrap.php` stubs make `MemberService::registerNewMember()` itself fully unit-testable, but the admin-post handler wiring in `PortalModule` is not directly tested, consistent with this codebase's standing Shortcode/admin-post precedent) - needs a real staging pass, same caveat as every other public-facing form in this project.

## Addendum: branded login shortcode

Follow-up ask: a login page for members. Until now the only way to sign in was WordPress's own `wp-login.php` - functional, but wp-admin-styled and disconnected from Register/Portal, which already live as front-end shortcodes on the association's own pages.

**`[association_manager_login redirect="/my-account/"]`** (`Modules\Portal\Public\LoginShortcode`) is the third shortcode in this same family, same shape as `RegistrationShortcode`: a `redirect` attribute, a random-token-keyed transient for round-tripping an error back to an anonymous visitor (`am_login_error_{token}`, same pattern as `am_register_error_{token}`), rendered via `templates/public/login.php`. It deliberately does **not** reinvent authentication - `PortalModule::handleLogin()` is a thin wrapper around plain `wp_signon()`, which already sets the current user and auth cookie on success (unlike `handleRegister()`, which calls `wp_insert_user()` and has to set both explicitly). "Forgot your password?" links straight to WordPress's own `wp_lostpassword_url()` rather than building a second, member-facing self-service reset flow - the admin-triggered reset email (`MembersModule::handleSendPasswordReset()`, same ADR-023 addendum family) covers the case where the secretariat sets a member up; a member who simply forgot their own password is exactly what WP core's existing lost-password flow is for, and duplicating it wasn't asked for.

No new tests: `handleLogin()` is admin-post glue calling straight into `wp_signon()`, same untested-by-design category as `handleRegister()`/`handleSubmitProfile()` per this ADR's own established precedent above. 268 tests still passing; `composer stan`/`composer cs` both clean.
