...
final class NotificationsModule implements ModuleInterface
{
    ...
    public function boot(Container $container): void
    {
        ...
        add_action(self::QUEUE_CRON_HOOK, static function () use ($queueRunner): void {
            $queueRunner->run();
        });
        ...
        add_action(
            ...
            function ($member, ?string $oldStatus, string $newStatus) use ($dispatcher): void {
        ...
        add_action(
            ...
            function ($member, string $newExpiresAt) use ($dispatcher): void {
                $dispatcher->notify(
                    'membership_renewed',
        ...
        add_action('rest_api_init', function () use ($templates): void {
     ...
     */
    private function resolveMemberEmail($member): ?string
    {
    ...
    private function handleSaveNotificationTemplate(NotificationTemplateRepositoryInterface $templates): void
    {
        ...
        wp_safe_redirect(add_query_arg(
            ['page' => NotificationTemplatesPage::SLUG, 'event_key' => $eventKey, 'am_notice' => 'saved'],
            admin_url('admin.php')
        ));
        exit;
    }
