...
final class MembersController {
    ...
    public function create( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        if ( $customFields !== [] ) {
            $this->fieldValueService->save( 'member', $member->requireId(), $customFields );
        }
    ...
    public function updateFields( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        $updated = $this->service->find( $memberId );

        if ( $updated === null ) {
            return new WP_Error( 'am_member_not_found', 'Member not found.', [ 'status' => 404 ] );
        }

        return new WP_REST_Response( $this->toArray( $updated ), 200 );
    }
    ...
    public function suspend( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        return $this->transition(
            $request,
            fn ( int $id ): Member => $this->service->suspendMember(
                $id,
                $this->currentUserId(),
    ...
    public function renew( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        ...
        return $this->transition(
            $request,
            fn ( int $id ): Member => $this->service->renewMembership(
                $id,
                $expiresAt,
                $this->currentUserId()
            )
    ...
    public function renewByPlan( WP_REST_Request $request ): WP_REST_Response|WP_Error {
        return $this->transition(
            $request,
            fn ( int $id ): Member => $this->service->renewMembershipByPlan(
                $id,
                $this->currentUserId()
            )
        );
    ...
    private function toArray( Member $member ): array {
        return [
            ...
            'custom_fields'   => $this->fieldValueService->valuesFor( 'member', $member->requireId() ),
        ];
    }
}
